// FOR/FOR-OF  AND .PUSH()
// Challenge: write a function that uses the for/for-of Statements & the Array.prototype.push() methods within it. Using the for loop and then again the for-of statement, loop over all of the presidents from our presidents array. Then, push the president's last name and the year they were born to a new array as individual objects containing key/value pairs of each presidents last name and date of birth. Then, return the new array from the function.
...
let fnReduce = () => {
    ...
    let count = arr.reduce((accumlator, currentValue) => {
        return (accumlator += currentValue.value);
    }, 0);
    return count;
};
...
let fnFilter = () => {
    let filtered = presidents.filter(
        (president) => president.year > 1900 && president.passed === undefined
    );
    return filtered;
};
